...
public class FluentUserStories extends AbstractMultiThreadedTest {
    private static final Logger LOG = LoggerFactory.getLogger(FluentUserStories.class);
    ...
    public void learning() {
        ...
        for (int index = 0; index < 100; index++) {
            log().info("{} % 10 == 0 -> {}", i.intValue(), i.intValue() % 10 == 0);
            i.incrementAndGet();
        }
    ...
    public void invocationCount() throws Exception {
        final AtomicInteger i = new AtomicInteger(0);
        test().setup(setup().threads(3).invocations(10).build()).executable(() -> {
            executor().execute();
            i.incrementAndGet();
        }).start();
    ...
    @Test
    public void durationMultiThreadSleepingMoreThanOneSecondExecutable() throws Exception {
        test().setup(
                setup().duration(Duration.seconds(15)).threads(10).invocationRange(1000).throughputRange(30).build())
    ...
    public void customSummaryAppender() throws Exception {
        test().setup(setup().duration(Duration.seconds(15)).threads(10).invocationRange(1000).throughputRange(30)
                ...
                .build()).executable(() -> Thread.sleep(RANDOM.nextInt(1000))).start();
    }
}
