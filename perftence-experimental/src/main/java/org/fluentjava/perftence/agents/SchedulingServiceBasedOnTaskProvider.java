...
final class SchedulingServiceBasedOnTaskProvider implements TestTaskSchedulingService {
    ...
    public SchedulingServiceBasedOnTaskProvider(final TaskProvider taskProvider, final RunnableAdapter runnableProvider,
            final int workerAmount, final TestFailureNotifier testFailureNotifier) {
    ...
    @Override
    public void scheduleTask(final TestTask task) throws ScheduleFailedException {
    ...
    @Override
    public void scheduleFirstTasks(final Collection<TestAgent> agents) throws ScheduleFailedException {
        if (agents.isEmpty()) {
    ...
    private void validateWorkerAmount(final int agents) throws ScheduleFailedException {
        if (workerAmount() == -1) {
            workerAmount(agents);
        }
        if (workerAmount() == 0) {
    ...
    private void firstTaskScheduling(final Collection<TestAgent> agents) throws ScheduleFailedException {
    ...
    private void addNewWorkerThread() {
        threads().add(threadFactory()
                .newThread(addWorker(new Worker(taskProvider(), runnableProvider(), failureNotifier()))));
    }
