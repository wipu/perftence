...
final class Worker implements Runnable {
    ...
    public Worker(final TaskProvider taskProvider, final RunnableAdapter runnableProvider,
    ...
    public void run() {
        while (true) {
            try {
                ...
                final ScheduledTestTask task = taskProvider().waitForNextScheduledTask();
                if (task == null) {
    ...
    private void run(final ScheduledTestTask task) throws ScheduleFailedException {
        runAndMeasure(task);
    ...
    private void scheduleNextTask(final TestTask task) throws ScheduleFailedException {
