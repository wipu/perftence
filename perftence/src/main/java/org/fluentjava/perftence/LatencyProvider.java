...
public final class LatencyProvider implements StatisticsProvider, RuntimeStatisticsProvider, TestTimeAware {
    ...
    static LatencyProvider withSynchronized() {
        return new LatencyProvider(StronglyTypedSortedBag.<Long> synchronizedTreeBag());
    }
    ...
    public double throughput() {
        if (this.startTime == -1 || this.endTime == -1) {
            throw new IllegalStateException("Invalid state: Use start() and stop() to indicate test start and end!");
        }
        return calculateThroughput(sampleCount(), duration());
    }

    private static double calculateThroughput(final long sampleCount, final long duration) {
